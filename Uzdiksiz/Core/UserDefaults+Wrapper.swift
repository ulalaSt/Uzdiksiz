...
protocol UserDefaultsRepresentableDecoded: UserDefaultsRepresentable, Codable {
}

extension UserDefaultsRepresentableDecoded {
    var rawDefaultsValue: Data {
        (try? JSONEncoder().encode(self)) ?? Data()
    }
    
    init?(rawDefaultsValue: Data) {
        if let object = try? JSONDecoder().decode(Self.self, from: rawDefaultsValue) {
            self = object
        }
        return nil
    }
    ...
    public static func object(forKey defaultName: String, from userDefaults: UserDefaults) -> Self? {
        if let data = userDefaults.object(forKey: defaultName) as? Data,
